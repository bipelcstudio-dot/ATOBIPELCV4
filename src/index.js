...
async function tableExists(env,t){
  try{
    return !!(
      await env.DB
        .prepare(
...
async function auth(req,env){
  ...
  if(!(await tableExists(env,'sessions')))return null;
...
function can(u,p,need){
  return !!u &&
    (
      ALL.has(u.role) ||
      p.includes(need) ||
      p.includes(`${need}.view`) ||
      p.includes(
        p===need
          ? need
          : `${need}.all`
...
async function log(env,u,action,type,eid){
...
async function dashboard(env){
  ...
  await safe(
    'customerDebt',
    `
      SELECT COALESCE(SUM(amount-paid_amount),0)n
      FROM invoices
      WHERE status!='Paid'
    `
  );
...
async function create(env,u,r,b){
  ...
  if(r==='users'){
    ...
    if(
      ...
      b.employment_date=b.start_date;
    }
  }
  ...
  try{
    ...
    return json({
      error:e.message||'ثبت انجام نشد'
    },400);
  }
}

async function update(env,u,r,idv,b){
  ...
  if(!keys.length){
    return json({
      error:'داده‌ای برای ویرایش نیست'
    },400);
  }
  ...
  try{
    ...
    return json({
      error:e.message||'ویرایش انجام نشد'
    },400);
  }
}
...
async function handle(req,env){
  ...
  if(
    ...
    if(!countRow){
      ...
      return json({
        error:
          'جدول users وجود ندارد؛ ابتدا migration اولیه (schema.sql) را اجرا کنید'
      },500);
    }
   ...
   *
   * IMPORTANT:
   * There is ONLY ONE login block here.
   */
